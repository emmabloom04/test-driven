...
if (!DB_URL) {
  throw new Error(
    "DB_URL environment variable is required. Create a .env file with DB_URL=postgresql://user:password@host:port/database",
  );
}
...
const pool = new Pool({
  ...
  ssl: {
    ...
    checkServerIdentity: () => {
      return undefined;
    }, // Skip hostname verification but keep cert chain validation
  },
});
